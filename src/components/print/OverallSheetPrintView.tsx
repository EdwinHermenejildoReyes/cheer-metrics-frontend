...
export function OverallSheetPrintView(p: OverallPrintData) {
  ...
  const maxDanceDiff = p.danceDiffLevels.length > 0 ? Math.max(...p.danceDiffLevels.map(l => l.value)) : 0;
  ...
  return (
    <div className="hidden print:block" style={{ fontFamily: 'sans-serif', fontSize: '9.5px', color: '#18181b', lineHeight: 1.35 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: primary, color: primaryText, padding: '7px 14px', marginBottom: '7px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '8.5px', opacity: 0.75, margin: 0 }}>Planilla de Puntuación · Overall (General)</p>
          </div>
        </div>
      ...
      <div style={{ padding: '0 14px 5px', borderBottom: `2px solid ${primary}`, marginBottom: '6px' }}>
        ...
        {p.divisionName && <p style={{ fontSize: '9px', color: '#71717a', margin: '1px 0 0' }}>{p.divisionName} · Overall (General)</p>}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '14px', padding: '0 14px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ border: '1px solid #e4e4e7', borderRadius: '8px', padding: '10px 12px', backgroundColor: '#fafafa' }}>
            ...
            <p style={{ fontSize: '8.5px', color: '#a1a1aa', margin: '0 0 6px' }}>−0.1 por cada problema de espaciado o choque/empalme en transiciones</p>
              ...
              {FORMATIONS_VALUES.map(v => {
                ...
                return (
                  <div key={v} style={{
                    ...
                    {errors > 0 && <span style={{ fontSize: '7px', opacity: 0.7 }}>−{errors}</span>}
                  </div>
                );
              })}
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px', padding: '3px 6px', backgroundColor: p.errorsCount > 0 ? '#fef2f2' : `${primary}15`, borderRadius: '4px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 12px', backgroundColor: primary, color: primaryText, borderRadius: '8px' }}>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', fontSize: '9.5px' }}>
            ...
            <span style={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums', fontSize: '14px' }}>{fmt(p.overallSubtotal)}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', backgroundColor: primary, color: primaryText, borderRadius: '8px' }}>
            <div>
              <p style={{ fontWeight: 700, fontSize: '11px', margin: 0 }}>TOTAL PLANILLA OVERALL</p>
              <p style={{ fontSize: '8.5px', opacity: 0.7, margin: '2px 0 0' }}>
                {p.isEscolarAB
                  ? `General ${fmt(p.overallSubtotal)} + Cheer ${fmt(p.showmanshipOverall)}`
                  : `General ${fmt(p.overallSubtotal)} + Creat. ${fmt(p.creativityOverall)} + Show. ${fmt(p.showmanshipOverall)}`}
              </p>
            </div>
          ...
          {p.finalScore && (
            <div style={{ border: '1px solid #e4e4e7', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ padding: '8px 12px 6px', borderBottom: '1px solid #e4e4e7' }}>
                <p style={{ fontSize: '8.5px', fontWeight: 700, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>Estadísticas de la Competencia</p>
              </div>
              <table style={{ width: '100%', fontSize: '9.5px', borderCollapse: 'collapse' }}>
                <tbody>
                  <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                    <td style={{ padding: '4px 10px', color: '#52525b' }}>Puntaje bruto</td>
                    <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>
                      {parseFloat(p.rawScore ?? '0').toFixed(2)} / {parseFloat(p.maxRaw ?? '0').toFixed(2)}
                    </td>
                  </tr>
                  <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                    <td style={{ padding: '4px 10px', color: '#52525b' }}>Puntaje escalado</td>
                    <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{parseFloat(p.scaledScore ?? '0').toFixed(2)}</td>
                  </tr>
                  {parseFloat(p.totalDeductions ?? '0') > 0 && (
                    <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                      <td style={{ padding: '4px 10px', color: '#dc2626' }}>Total deducciones</td>
                      <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600, color: '#dc2626' }}>−{parseFloat(p.totalDeductions!).toFixed(2)}</td>
                    </tr>
                  )}
                  <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                    <td style={{ padding: '4px 10px', color: '#52525b' }}>Porcentaje del máximo</td>
                    <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{p.percentage}%</td>
                  </tr>
                  <tr style={{ backgroundColor: primary }}>
                    <td style={{ padding: '5px 10px', fontWeight: 700, fontSize: '11px', color: primaryText }}>Puntaje Final</td>
                    <td style={{ padding: '5px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 700, fontSize: '14px', color: primaryText }}>{parseFloat(p.finalScore).toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div style={{ border: '1px solid #e4e4e7', borderRadius: '8px', overflow: 'hidden', backgroundColor: '#fafafa' }}>
            ...
            <table style={{ width: '100%', fontSize: '9.5px', borderCollapse: 'collapse' }}>
              <tbody>
                <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                  <td style={{ padding: '4px 10px', color: '#52525b' }}>Formaciones y Transiciones</td>
                  <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                    <span style={{ fontWeight: 700 }}>{fmt(p.formationsScore)}</span>
                    <span style={{ color: '#a1a1aa' }}> / 2.00</span>
                  </td>
                </tr>
                <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                  <td style={{ padding: '4px 10px', color: '#52525b' }}>Dificultad de Baile</td>
                  <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                ...
                <tr style={{ backgroundColor: `${primary}12`, borderBottom: '1px solid #f4f4f5' }}>
                  <td style={{ padding: '4px 10px', fontWeight: 700, color: primary }}>Subtotal General</td>
                  <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 700, color: primary }}>{fmt(p.overallSubtotal)}</td>
                </tr>
                {!p.isEscolarAB && (
                  <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                    <td style={{ padding: '4px 10px', color: '#52525b' }}>Creatividad (este juez)</td>
                    <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                      <span style={{ fontWeight: 700 }}>{fmt(p.creativityOverall)}</span>
                      <span style={{ color: '#a1a1aa' }}> / 2.00</span>
                    </td>
                  </tr>
                )}
                <tr style={{ borderBottom: '1px solid #f4f4f5' }}>
                  <td style={{ padding: '4px 10px', color: '#52525b' }}>{p.isEscolarAB ? 'Cheer / Animación (este juez)' : 'Showmanship (este juez)'}</td>
                  <td style={{ padding: '4px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                    <span style={{ fontWeight: 700 }}>{fmt(p.showmanshipOverall)}</span>
                    <span style={{ color: '#a1a1aa' }}> / {fmt(p.showmanshipMax)}</span>
                  </td>
                </tr>
                <tr style={{ backgroundColor: primary }}>
